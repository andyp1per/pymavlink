...
def fft(logfile):
    ...
    while True:
        ...
        if m is None:
            break
        type = m.get_type()
        if type.startswith("GYR1"):
            data[type+'.GyrX'].append(m.GyrX)
            data[type+'.GyrY'].append(m.GyrY)
            data[type+'.GyrZ'].append(m.GyrZ)
        if type.startswith("FTN3"):
            data[type+'.GyrX'].append(m.GyrX)
            data[type+'.GyrY'].append(m.GyrY)
            data[type+'.GyrZ'].append(m.GyrZ)

    print("Extracted %u data points" % len(data['FTN3.GyrX']))

    for msg in ['FTN3', 'GYR1']:
        pylab.figure()

        if msg.startswith('FTN'):
            prefix = 'Gyr'
        else:
            prefix = 'Gyr'
        for axis in ['X', 'Y', 'Z']:
